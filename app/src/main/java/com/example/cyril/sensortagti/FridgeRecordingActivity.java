 ...
 */
public class FridgeRecordingActivity extends Activity
{

    private final static String TAG = FridgeRecordingActivity.class.getSimpleName();
    // EXTRA.
    ...
    private final BroadcastReceiver mGattUpdateReceiver = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            } else if (BluetoothLeService.ACTION_GATT_SERVICES_DISCOVERED.equals(action))
            {
                ...
                s1=new MotionSensor(motionUuid,mBluetoothLeService,mDeviceAddress);
                UUID irtUuid=UUID.fromString(SensorTagGattAttributes.UUID_IRT_CONF);
                s3=new IRTSensor(irtUuid,mBluetoothLeService,mDeviceAddress);
    ...
    private void onCharacteristicChanged(String uuidStr,byte[] value)
    {
        ...
        {
            this.thermometer=this.s3.convert(value);
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.fridge_recording);
        ...
        this.thermometer=new Point3D(0.0,0.0,0.0);
    }
     ...
     */
    CountDownTimer timer1=new CountDownTimer(1250,100)
    {
        ...
        public void onFinish()
        {
            ...
            if(X.get(106)<=3.6288)
            {
                if(X.get(25)<=-0.0183)
                {
                    mAction.setText("No action ("+X.get(106)+"<=3.6288) ("+X.get(25)+"<=-0.0183)");
                }
                else
                {
                    mAction.setText("In use ("+X.get(106)+"<=3.6288) ("+X.get(25)+">-0.0183)");
                }
            }
            else
            {
                if(X.get(67)<=1.5)
                {
                    mAction.setText("In use ("+X.get(106)+">3.6288) ("+X.get(67)+"<=1.5)");
                }
                else
                {
                    if(X.get(97)<=-3.0289)
                    {
                        mAction.setText("Vibrating ("+X.get(106)+">3.6288) ("+X.get(67)+">1.5) ("+X.get(97)+"<=-3.0289)");
                    }
                    else
                    {
                        mAction.setText("In use ("+X.get(106)+">3.6288) ("+X.get(67)+">1.5) ("+X.get(97)+">-3.0289)");
                    }
                }
            }
    ...
    {
        // Initialize the relevant recorders.
        ArrayList<Point3D> irtRecorder=new ArrayList<>();
        @Override
        public void onTick(long millisUntilFinished)
        {
            // Add measures to respective containers.
            irtRecorder.add(thermometer);
        }
        @Override
        public void onFinish()
        {
            // Get the change in the measurements (derivative).
            ArrayList<Point3D> irtChanges=new ArrayList<>();
            Statistics.getChanges(irtRecorder,irtChanges);
            boolean strInc=true,strDec=true;
            for(int i=0;i<irtChanges.size();i++)
            {
                if(irtRecorder.get(i).x<0)strInc=false;
                else if(irtRecorder.get(i).x>0)strDec=false;
            }

            // Get the min in the measurements.
            Point3D min=Statistics.min(irtRecorder);
