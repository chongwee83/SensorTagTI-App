 ...
 */
public class StoveRecordingActivity extends Activity
{

    private final static String TAG = StoveRecordingActivity.class.getSimpleName();
    // EXTRA.
    ...
    public void onCreate(Bundle savedInstanceState)
    {
        ...
        setContentView(R.layout.stove_recording);
    ...
    CountDownTimer timer=new CountDownTimer(5000,100)
    {
        ...
        public void onFinish()
        {
            ...
            // Get the max in the measurements.
            Point3D max=Statistics.max(irtRecorder);

            // Predict.
            if(!strInc&&!strDec)
            {
                if(max.x<28)
                    mState.setText("Closed "+"(stable - "+ max.x+"<28");
                else
                    mState.setText("Opened "+"(stable - "+ max.x+">28");
            }
            else if(strInc)
            {
